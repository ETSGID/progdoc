...
exports.getAsignaturasPD = async function (req, res, next) {
    ...
    try {
        let pdis = await progDocController.getProgramacionDocentesAnteriores(req.params.plan, req.params.semestre, req.params.anoAcademico, null, null)
        //debo comprobar el año ya que getProgramacionDocnetesAnteriores te da la ultima cerrada o anteriores
        switch (pdis.length) {
            case 1:
                if (progDocController.getAnoPd(pdis[0].identificador) === req.params.anoAcademico) pdID1 = pdis[0].identificador;
                break;
            case 2:
                if (progDocController.getAnoPd(pdis[0].identificador) === req.params.anoAcademico) pdID1 = pdis[0].identificador;
                if (progDocController.getAnoPd(pdis[1].identificador) === req.params.anoAcademico) pdID2 = pdis[1].identificador;
                break;
            default:
                break;
        }
...
exports.getGruposAsignatura = async function (req, res, next) {
    ...
    try {
        ...
        let grupos = await models.sequelize.query(query = `SELECT distinct a.identificador, a.codigo, a.nombre as "nombreAsignatura", a.acronimo, g.nombre, g."nombreItinerario", g.aula, g.capacidad, g."nombreItinerario", a."anoAcademico"
