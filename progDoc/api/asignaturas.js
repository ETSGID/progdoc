...
exports.getAsignaturasPD = async function (req, res, next) {
  ...
  try {
    ...
    const pdis = await progDocController.getProgramacionDocentesAnteriores(req.params.plan, req.params.semestre, req.params.anoAcademico, null, null);
    // debo comprobar el año ya que getProgramacionDocnetesAnteriores te da la ultima cerrada
    // o anteriores
    switch (pdis.length) {
    case 1:
      if (progDocController.getAnoPd(pdis[0].identificador) === req.params.anoAcademico) {
    ...
    case 2:
      ...
      if (progDocController.getAnoPd(pdis[1].identificador) === req.params.anoAcademico) {
...
exports.getGruposAsignatura = async function (req, res, next) {
  ...
  try {
    ...
    const grupos = await models.sequelize.query(`SELECT distinct 
    ...
    {
      replacements: {
        pdID1, pdID2, semestre: semestreGrupo, codigo: req.params.codigoAsignatura,
      },
    });
    const asignatura = {};
    asignatura.nombre = '';
