...
exports.getEstado = async function (req, res, next) {
  ...
  if (!res.locals.progDoc || !res.locals.departamentosResponsables) {
    const view = req.originalUrl.toLowerCase().includes('consultar') ? 'estados/estadoConsultar' : 'estados/estadoCumplimentar';
    res.render(view, {
    ...
    try {
      ...
      if (res.locals.departamentosResponsables.length > 0) {
        departamentoID = req.session.departamentoID ? req.session.departamentoID
