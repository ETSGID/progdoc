...
async function getNotasGruposSinAsignatura(gruposBBDD) {
  ...
  try {
    ...
    notas.forEach((nota) => {
      ...
      n.asignaturaAcronimo = gruposBBDD.find((obj) => obj.grupoId === nota.GrupoId).nombre;
      n.asignaturaNombre = 'Nota de grupo';
      n.asignaturaIdentificador = 'grupo';
      n.asignaturaCodigo = gruposBBDD.find((obj) => obj.grupoId === nota.GrupoId).nombre;
...
exports.getHorario = async function (req, res, next) {
  ...
  if (!res.locals.progDoc || !res.locals.departamentosResponsables) {
    const view = req.originalUrl.toLowerCase().includes('consultar') ? 'horarios/horariosConsultar' : 'horarios/horariosCumplimentar';
    res.render(view, {
      ...
      async function getAsignacionHorario(ProgramacionDocenteIdentificador) {
        ...
        */
        ...
        horarioAsignaturas.forEach((horarioAsignatura) => {
          const GrupoNombre = horarioAsignatura['AsignacionProfesors.Grupo.nombre'];
          // lo convierto en string
          let c = asignacionsHorario.find((obj) => obj.curso === horarioAsignatura.curso);
          ...
          if (!c) {
            ...
            c = asignacionsHorario.find((obj) => obj.curso === horarioAsignatura.curso);
          }
          let asign = asignaturas.find((obj) => obj.nombre === horarioAsignatura.nombre);
          if (!asign) {
            asign = {};
          ...
          if (GrupoNombre) {
            const s = c.semestres.find((obj) => obj.semestre === Number(GrupoNombre.split('.')[1]));
            if (s) {
              // busco el grupo ya se inició
              const g = s.grupos.find((obj) => obj.grupoId === horarioAsignatura['AsignacionProfesors.GrupoId']);
    ...
    try {
      ...
      if (toEliminar) {
        ...
        toEliminar.forEach((element) => {
          ...
          if (element.split('_').length === 7) {
            ...
            const asig = asignaturaAsignacions.find((obj) => (asignacion && obj['AsignacionProfesors.identificador'] === asignacion));
      ...
      if (toAnadir) {
        ...
        toAnadir.forEach((element) => {
          ...
          const asig = asignaturaAsignacions
            .find(
              (obj) => (
                nuevaEntrada.AsignaturaId && obj.identificador === nuevaEntrada.AsignaturaId),
            );
          if (!asig) {
        ...
        const promise2 = models.AsignacionProfesor.bulkCreate(
          queryToAnadir,
        );
...
exports.guardarNota = async function (req, res) {
  if (!res.locals.permisoDenegado) {
    try {
      ...
      const nToAnadir = models.AsignacionProfesor.build(
        notaToAnadir,
      );
...
exports.aprobarHorarios = async function (req, res, next) {
  ...
  let estadoHorarios;
  try {
    const pd = await models.ProgramacionDocente.findOne({ where: { identificador: pdID }, attributes: ['estadoHorarios'] });
